...
- **Unit analyzer 实现物理归并（2026-07-28 锁定）**：R5 的 facade-only 收敛继续推进到资源归属一致：四套 canonical 实现统一放在 `.agents/skills/unit-analyst/scripts/{paper,repo,dataset,blog}.py`，所有新运行时路由只从一个 kind registry 取得 canonical script、prepare operation、id argument 与持久 owner。物理移动不得迁移 `paper/repo/dataset/blog-analyst` 的 record owner、preference skill/operation、journal operation、ConfirmationReceipt、provenance 或 diagnostic identity，也不得重写既有 KB。旧四目录只允许保留无业务逻辑、无判断、无 schema 的薄兼容启动器，在当前 RC 升级窗口转发历史命令到 canonical script；新输出、文档、测试与安装 manifest 一律指向 `unit-analyst/scripts/`。兼容启动器只能在确认已安装版本和持久产物不再引用旧路径后另行移除。
