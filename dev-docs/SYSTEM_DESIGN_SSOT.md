...
> **状态（2026-07-28）**：**`0.2.0-rc.7` 本地可用门已完成**。R17–R26 的严格 snapshot/确认、恢复与更新、Agent-led portfolio/偏好路由、完整 survey、无插件 Obsidian 多项 review、provider-neutral literature/monitor、正式 report/portfolio publication、公开 owner loader 与离线 Agent-plan runtime binding已闭环；随后完成 R5 发现面收敛与 `unit-analyst` 资源彻底归并。最终主线 `2317 passed, 18 skipped`，15/15 skill validator、62-file Python 3.9 AST、shell/diff、安装生命周期、版本一致性与无 API Key/付费/插件发布门均绿。它仍不是 stable/GA；真实外部来源、Obsidian Reading-view 与 hosted Linux/macOS CI 是 tag 前置，push/tag/publish 不自动执行。canonical KB、不可变 evidence、确认门、精确恢复与用户只见自然语言 + `kb <verb>` 等红线不变；实时施工状态写 `dev-docs/BACKLOG.md`。
> **这份文件是什么**：从今往后，系统的**意图 / 目标架构 / 功能设计**以本文件为唯一可信源。
> 它描述的是**目标形态**（吸收审查后系统该长成什么样），不是当前 shipped 状态。`dev-docs/` 是维护者本地、Git-ignored 的设计与施工工作台；每一块**落地后**必须把稳定合同 graduate 进 tracked 的 `AGENTS.md`、`CONTRIBUTING.md`、`docs/DESIGN.md`、`SCHEMAS.md` 或 `CHANGELOG.md`，不能让 public clone 依赖本目录才能理解当前产品。

### 文档与版本库边界（2026-07-28 锁定）

- `dev-docs/` 保存 SSOT 草案、backlog、一次性 handoff、审查原始记录和历史验收快照；它可以包含本机路径、瞬时 commit、测试日志与已过时的物理布局，因此不得进入发布包，也不再由 Git 跟踪。退出跟踪只用 index removal，保留维护者本地文件；不为此改写公共 Git 历史。
- Git 必须跟踪所有当前合同：根 `README/CONTRIBUTING/SECURITY/CHANGELOG/AGENTS`、`docs/`、分发树 `.agents/*.md`、每个 discoverable `SKILL.md`、共享 schema 与必要 reference。面向用户、安装后 Agent 和贡献者的当前事实不得只存在于 `dev-docs/`。
- 历史 handoff/audit 不做“追平当前路径”的机械改写；它们是时间点证据。当前文档不得引用它们作为运行前置。文档漂移门必须从实际 skill inventory 生成期望，至少拒绝已删除物理目录、过时 skill 数量和已退出 shipping 面的 `wiki-adapter`/`research-navigator`。
...
- 用户偏好只有一个 canonical SSOT：workspace profile + runtime preferences；**不按 skill 复制会独立漂移的偏好副本**。
...
- `knowledge-base-manager`（机械 schema/lifecycle）、`research-config-manager`（偏好事实 owner）、`discussion-archivist`（搬运调用方已写内容）、`unit-analyst`（发现 facade；偏好由四个持久 implementation identity 消费）与 `skill-evolution-advisor`（治理/诊断）显式声明 preference-neutral，eligible allowlist 为空并记录原因；其 hard safety/governance 开关仍由对应 owner 直接强制，不伪装成 soft preference consumer。中央测试必须同时证明：shipping skill 要么有至少一个真实 operation，要么在 neutral registry 中有非空理由，二者互斥；不得存在“有候选但永远无法选”的第三种状态。
...
- 默认顺序全量测试、Python 3.9/3.11/3.12 兼容、当前 discoverable-skill validator、compileall、shell syntax、diff、安装/update/reinstall/uninstall/undo/restore、无工具/离线降级必须全绿。新增、合并或删除 skill 后，文档、计数、manifest allowlist 与 release tests 必须从同一实际 inventory 同步。
...
- 建立“当前 shipping owner × preference-sensitive operations”真实 consumer 矩阵测试：不仅验证 allowlist 存在，还验证未选 soft preference 不影响输出、错 skill/op/task receipt 被拒、canonical preference 改变令结果 stale。公开 CHANGELOG/README 只有在矩阵覆盖真实 consumer 后才可声称“全部接入”。
...
- **canonical schema（锁定规格，Codex 实现进 SCHEMAS.md + `evidence.py`）**：
  ...
  - **降级来源升级（2026-07-27 冷验收锁定）**：当既有 canonical unit 只保存 degraded 摘要/页面壳，且仍未形成 verified/confirmed 判断时，用户随后提供同一强身份的完整本地 PDF/HTML，duplicate 不能以 exit-0 静默吞掉恢复意图。系统必须保留旧 unit 与全部 raw/derived bytes 不变，另建新 canonical revision，并在一个事务中把旧 unit 标为 archived、双向写入 `superseded_by` / `supersedes` link，随后把分析管线指向新 unit。完整性门判断的是**原始材料与可读语义覆盖是否完整**，不能把所有 converter warning 等同于摘要壳：PDF 若保留完整 raw bytes、稳定逐页 locator，并产生至少 2 页且总计至少 4,000 字符的实质 parse，允许在保留 `degraded` 状态和全部警告的前提下用于 revision upgrade（例如少量页面启用 native-text recovery 或表格 Markdown 格式告警）；不得把它伪报为 warning-free `complete`。若旧 unit 已有 verified/confirmed 判断、来源身份不足以证明相同，或新材料未通过上述实质完整性门，则 fail-closed 请求用户决策，不自动替换、迁移确认或覆盖证据。
...
- **发现面 15、L1 owner 14**：最终 `.agents/skills/` 只发现 `kb-cli` + 14 个 owner skill。R5 先把四个来源 analyzer 在用户发现/路由层合并成 `unit-analyst`，当时原目录暂留 script implementation namespace；下条 2026-07-28 决策继续删除该过渡 namespace。底层 record owner、ConfirmationReceipt、preference operation 与逻辑 implementation identity 不迁移；`unit-analyst` 按 kind 显式映射这些持久身份，避免破坏 currentness、receipt 与诊断历史。`skill_directories()` 只枚举含 `SKILL.md` 的目录。
- **Unit analyzer 实现物理归并（2026-07-28 锁定，按用户反馈收紧）**：R5 的 facade-only 收敛继续推进到资源归属一致：四套 canonical 实现统一放在 `.agents/skills/unit-analyst/scripts/{paper,repo,dataset,blog}.py`，所有运行时路由只从一个 kind registry 取得 canonical script、prepare operation、id argument 与持久 owner。物理移动不得迁移 `paper/repo/dataset/blog-analyst` 的 record owner、preference skill/operation、journal operation、ConfirmationReceipt、provenance 或 diagnostic identity，也不得重写既有 KB。`.agents/skills/{paper,repo,dataset,blog}-analyst/` 四个旧资源目录必须从源码与安装包彻底删除，不保留 launcher 或第二套 namespace；逻辑 owner 字符串只作为持久协议身份存在于 canonical 实现和共享 schema/registry 中。copy-install update 必须把旧 manifest 中的旧脚本作为受管 removal 原子删除并新增 canonical script，managed drift 仍 fail-closed。历史裸脚本命令不属于 RC 兼容承诺；当前运行时、文档、测试和真实 KB 均不得继续生成或保存旧路径。
