...
- **2026-07-28 文档/Git 边界审查进行中**：确认 `.agents/README.md` 仍保留 20-skill 与已删除 owner 的旧清单；`SCHEMAS.md` 仍把退出 shipping 面的 navigator/wiki-adapter 写成当前 owner；根 `AGENTS.md` 仍指向已迁走的 `temp/`。`dev-docs/` 共 168 个 tracked 文件（132 个一次性 prompt，约 2.2 MB），不进入安装包但含本机路径与历史日志。设计已锁定为本地 Git-ignored 工作台：保留本地文件、从 index 移除，不重写历史；当前合同全部 graduate 到 tracked 公共/分发文档并增加 inventory 漂移门。
