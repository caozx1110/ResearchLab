...
- **G4 实验导入**：`experiment.py import-runs` 接受 wandb export JSON / CSV / 目录约定，批量生成 log-run（fingerprint 机制沿用，来源标 imported）。没有它，实验归档必然退化为手工口述。约 2–3 天。
  ...
  **R4 实现锁定（2026-07-27，experiment import）**：不新增公开 `kb` 动词；自然语言导入由 `experiment-workbench` 私有 `import-runs` operation 执行。runtime Agent 先把用户选定 export 放入 project-contained staging，owner 只接受普通文件或单层普通文件目录：W&B JSON 为 top-level list 或 exact `{runs:[...]}`；CSV 用稳定 header；目录只读按名称排序的 `run-*.json` 单对象。拒绝 symlink/special/nested entry、重复列/identity、非 UTF-8、非有限数、超 16 MiB/file、1000 runs 或 128 MiB/batch。原始 source bytes 按 batch digest 复制到 experiment 的 `imports/`，长期 provenance 只保存 project-relative artifact、byte digest、format、source row/file 与 external run id，不保存绝对机器路径。

  parser 只做事实 ETL：显式字段直接映射；W&B `config` 无显式 revision 时用 canonical config digest；numeric summary/`metric.*` 列成为 typed metric，未知方向保持 `unknown`；state→outcome 只走固定公开映射，未识别值为 `inconclusive`，不推断成功原因。每条 normalized run 保存 `source=imported` + item/batch binding，并复用现有 fingerprint/repeat/seed/config、artifact containment、comparison 与 reporting-event 合同。整批在一个 workspace lock + journal + exact target set 中预分配 run ids、校验全部 duplicate/conflict、写 raw import copy、N 个 run Markdown、一个 run-log/record/event/index，再做 post-write/commit aggregate currentness；任一行失败零业务写。重放相同 item digest 是幂等 skip；相同 external id 或 fingerprint+seed/config 指向不同 item 必须整批 fail closed，不能暗当 rerun。全批只建一个 checkpoint，用户输出只给 imported/skipped/conflict 计数和自然语言修复。
...
4. **报告与产出全量投入**：周报/工作报告编辑层（"进展/问题/下一步"叙事，≈2–3 天）+ 论文大纲→初稿+引用（G1+G2，≈1.5 周）+ PPT 素材差异化（每页一结论+证据+讲述顺序，≈2 天）+ 图表提取可引用化（G7，caption/编号索引，≈3 天）。
   ...
   **R4 实现锁定（2026-07-27，weekly/PPT editorial）**：保留现有公开自然语言面与既有 report verbs；新增的 prepare/verify 仅是 `report-author` 私有 owner operations。同一份 snapshot-bound `ReportInputs` 先生成 exact editorial manifest + hollow Agent fill，不由脚本写叙事。catalog 只暴露 current confirmed claims +逐字 evidence、允许进入 factual lane 的 events、current confirmed decisions 和 current figure-index refs；每个 ref 保存内容/evidence/receipt/figure byte binding，pending/stale judgement 只能作为显式风险提示，不能进入 formal support catalog。

   weekly fill 固定 `executive_summary / progress / problems_and_risks / next_steps` 四区，每条 Agent 文本必须绑定至少一个 catalog ref并保留 epistemic label；verify 后渲染可直接交付的中文默认周报，正文是“本周摘要→进展→问题与风险→下周计划”，末尾机械附完整 evidence appendix。PPT fill 是 1–12 个顺序 slide，每页恰一条 Agent-authored conclusion、至少一个 evidence ref、可选 stable figure refs、speaker note 与 transition；若 catalog 有 current figure，整套至少引用一张，否则显式标 missing。渲染固定“结论→证据→图示→讲述/过渡”而不是周报段落，也不复用 outline 七节结构。raw markup/LaTeX、未知/重复 ref、空壳、绝对路径与超限文本 fail closed。

   **R4 冷验收补强（2026-07-27）**：成功 diagnosis 与 editorial verify 必须 checkpoint 本轮 Agent fill 和全部 product-owned outputs；否则 audit 会把“功能成功但恢复未闭合”判为失败。shared reporting-event writer 必须为新事件持久生成 stable reference-safe id，不能让正常 factual event 因无 identity 降成 risk。report/bib/figure catalog 面对 paper+experiment 等混合 program 时先做 globally unique kind-neutral record resolution，再仅消费 paper；非 paper 不是错误，缺失/歧义/unsafe identity 才 fail closed。R4 验收必须含混合 program，不能只用 paper-only fixture。

   manifest 绑定 program state/events、canonical sources/receipts/evidence、decisions、figure index/assets、task-bound language/style receipt 与 output kind。prepare 不覆盖已编辑 current fill；verify 在 render 前、write 后、transaction commit boundary 聚合重验，stale 时不覆盖旧成品。周报与 PPT 是派生报告，不创造新的研究 judgement或自签；Agent 的综合必须通过逐项 refs 透明标为基于哪些 current 输入。一次自然语言请求中 Agent 自动完成 prepare→fill→verify，只在输入确实缺失或需要用户拍板时停。
