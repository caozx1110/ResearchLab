...
---
## §24 Stage 2 (install) — 施工中
### M13 解耦（先做，solo）
现状：dispatcher 的 `script_path()`(kb:118) 已自解耦（先试 `<kb-root>/.agents/skills`，回落 `DEFAULT_PROJECT_ROOT`=符号链接解析后 .agents/lib 所在=repo）。**真正残留的 runtime 硬绑**：
- `intake.py:81` auto-screen forward → `str(root/'.agents'/'skills'/'paper-analyst'/...)`
- `wiki.py:44` → `root/'.agents'/'skills'/'source-intake'/...`
- 打印的 confirm/hint 命令（common.py:31-34,83 / kb.py:48-52,86 / intake hints）—— 展示串。
设计：在 core.py 加 `skills_root()`（返回含 `.agents/skills` 的目录，按各脚本已有的 `__file__` walk-up 方式解析；加 `RESEARCH_SKILLS_HOME` env 覆盖）。把上述 runtime forwards 从 `root/.agents/skills` 换成 `skills_root()/.agents/skills`。dispatcher 统一走 skills_root()。加测试：kb-root 无 .agents 时 intake auto-screen / wiki 仍能找到 skills。
### M6/M12 install.sh + CLAUDE.md（M13 后）
symlink-not-copy 不变量；探测 ~/.claude ~/.codex；project/system scope；生成 CLAUDE.md=@AGENTS.md；smoke `kb help`+`kb status --root <ws>`；可选 kb 上 PATH。

### ✅ Stage 2 完成（merge 至 main）
- M13 解耦：skills_root()/RESEARCH_SKILLS_HOME（core+common），runtime forwards（intake/wiki）解绑 kb-root（merge 92d144c，160 tests，含 bare-workspace 测试）。
- M6/M12 install.sh + CLAUDE.md：symlink-not-copy 已验证、--dry-run 无副作用、CLAUDE.md 幂等 managed block、--uninstall 全反转、kb 在 RESEARCH_SKILLS_HOME 下 forward 正常。docs/INSTALL.md + README link（merge efe2134）。
- 剩 Stage 3（维护者亲自）：git archive HEAD → 新 git init → git fsck clean → push 新 remote。
- 遗留 nits：N7 散落 .DS_Store、N10 SKILL.md 示例 path-robust、kb init mkdir-before-forward（非阻塞）。
