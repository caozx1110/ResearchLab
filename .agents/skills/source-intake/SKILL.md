...
`literature-scout` 是本 skill 之前的薄发现 owner：它只把 OpenAlex fact metadata 写入同一 source-search staging，不创建 canonical paper，也不判断 relevance/quality。收到 scout stage 后，本 skill 继续拥有候选 review、去重与正式 materialization；stage id/source kind/normalized query 不可混用，候选按 OpenAlex ID 后 DOI 跨重跑折叠，相同查询重跑必须保留人工 status/note。引用量只能显示为来源 metadata。
