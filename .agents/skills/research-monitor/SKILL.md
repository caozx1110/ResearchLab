...
2. 私下创建 subscription。脚本使用带时区 anchor 机械计算 due，不解释研究内容。
3. 到期后创建一个冻结 run。同一过期窗口只创建一次；漏过多个周期合并为一次，不补出一串历史任务。
...
- run 冻结创建时的 target、scope digest、budget、cadence 与 subscription revision；后来暂停订阅不得篡改已开始 run。
