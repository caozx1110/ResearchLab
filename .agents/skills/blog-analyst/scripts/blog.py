...
def blog_preference_orientation(record: Mapping[str, object]) -> dict[str, object]:
...
def blog_preference_context(
    ...
    expected_orientation = blog_preference_orientation(record)
    ...
    if not isinstance(cache, dict) or _cache_unit_id(cache) != str(record.get("id") or ""):
        raise ValueError("canonical blog parse cache has a mismatched identity")
    ...
    source_binding = regular_tree_binding(
        unit_root / "source",
        logical_identity="blog-source-artifacts",
        trusted_root=root,
    )
    return {
        "canonical_id": str(record.get("id") or ""),
...
def _run_complete_note(args, root: Path, record: dict, unit_root: Path, defer_post_actions: bool) -> int:
    ...
    if args.phase == "prepare":
        ...
        orientation_path = unit_root / PREFERENCE_ORIENTATION_NAME
        write_yaml_if_changed(orientation_path, blog_preference_orientation(record))
        ...
        preference_task_context = blog_preference_context(root, record, unit_root)
        payload = build_note_scaffold(
            record,
            source_chunks,
            digest_chunks=12,
