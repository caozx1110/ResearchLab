(function () {
  const config = window.RESEARCH_NAVIGATOR_BROWSER || window.RESEARCH_KB_BROWSER || {};
  ...
  const TAB_CONFIG = [
    ...
    { id: "literature", label: "文献", shortLabel: "文", countKey: "literature_count", unit: "条" },
    ...
    { id: "wiki", label: "入口", shortLabel: "导", countKey: "wiki_item_count", unit: "项" },
  ];
  ...
  function renderQuickOpenBar() {
    ...
    const shortLabel = (item) => {
      ...
      if (title.includes("KB Index")) return "索引";
    ...
    const baseItems = [
      {
        ...
        title: "KB Index",
        subtitle: "index 预览",
        short_summary: snapshot.wiki_index_preview || "暂无 KB index 结果。",
        wiki_kind: "index",
      },
      {
        wiki_id: "wiki-log",
        title: "Current State",
        subtitle: "log 预览",
        short_summary: snapshot.wiki_log_preview || "暂无 current-state 结果。",
        wiki_kind: "log",
      },
      {
        wiki_id: "wiki-lint",
        title: "Navigation",
        subtitle: "lint 预览",
        short_summary: snapshot.wiki_lint_preview || "暂无 navigation 结果。",
        wiki_kind: "lint",
      },
    ];
    const queryCards = queryItems.map((entry, index) => ({
      wiki_id: `wiki-query-${entry.query_id || index}`,
      title: entry.title || entry.query_id || `entry-${index + 1}`,
      subtitle: entry.query_path || "kb/user",
      short_summary: entry.preview || "暂无入口预览。",
      wiki_kind: "query",
    ...
    const items = [
      {
        ...
        title: "查看文献区",
        subtitle: `${snapshotCount("literature_count")} 条`,
        short_summary: "按标题、作者、标签、主题快速检索论文与博客，并查看确认状态、摘要和来源链接。",
        tags: ["跳转到文献"],
        kind: "jump",
        jump_tab: "literature",
      },
      ...
      },
      {
        ...
        short_summary: "集中查看 KB index、current-state、navigation 与可重开的入口文件。",
        tags: ["跳转到入口"],
        kind: "jump",
        jump_tab: "wiki",
      },
    ...
    if (include("tags")) {
      (getSnapshot().tag_items || []).forEach((item) => {
        ...
        results.push({
          ...
          target_id: item.tag,
          title: item.tag,
          subtitle: `标签 · 文献 ${item.literature_count || 0} / 仓库 ${item.repo_count || 0}`,
    ...
    if (include("wiki")) {
      wikiItems().forEach((item) => {
        ...
        results.push({
          ...
          subtitle: `入口 · ${item.wiki_kind || "query"}`,
  ...
  function listCardHtml(item) {
    ...
    if (state.tab === "tags") {
      ...
      const subtitle = `${item.literature_count || 0} 条文献 · ${item.repo_count || 0} 个仓库`;
      return `
    ...
    if (state.tab === "landscapes") {
      ...
      const subtitle = `${item.scope || "scope 未标注"} · 文献 ${item.matched_literature_count || 0} · 仓库 ${item.matched_repo_count || 0}`;
      return `
  ...
  function renderOverviewDetail(item) {
    ...
    elements.detailBody.innerHTML = [
      section(
        "当前最值得看的 3 类内容",
        renderKeyValues([
          ["当前阅读", escapeHtml("优先看 reading list / 关键资料 note / source")],
      ...
      section(
        "工作区统计（精简）",
        renderKeyValues([
          ["文献", escapeHtml(String(snapshotCount("literature_count")))],
          ["仓库", escapeHtml(String(snapshotCount("repo_count")))],
          ["Program", escapeHtml(String(snapshotCount("program_count")))],
          ["入口条目", escapeHtml(String(snapshotCount("wiki_item_count")))],
        ])
      ),
    ].join("");
  }
  ...
  function renderTagDetail(item) {
    ...
    elements.detailMeta.textContent = `${item.literature_count || 0} 条文献 · ${item.repo_count || 0} 个仓库`;
  ...
  function renderWikiDetail(item) {
    const snapshot = getSnapshot();
    const queryItems = Array.isArray(snapshot.wiki_query_items) ? snapshot.wiki_query_items : [];
    elements.detailTitle.textContent = item.title || "入口";
    ...
    const queryListHtml = queryItems.length
      ...
      : "<li>暂无 query 结果。</li>";
    elements.detailBody.innerHTML = [
      section("Index 预览", `<p class="detail-copy">${escapeHtml(snapshot.wiki_index_preview || "暂无 KB index 结果。")}</p>`),
      section("Current State 预览", `<p class="detail-copy">${escapeHtml(snapshot.wiki_log_preview || "暂无 current-state 结果。")}</p>`),
      section("Navigation 预览", `<p class="detail-copy">${escapeHtml(snapshot.wiki_lint_preview || "暂无 navigation 结果。")}</p>`),
  ...
  function renderWorkbench() {
    ...
    document.title = wb.kind === "settings"
      ? "工作区设置 · Research Navigator"
      : wb.path
        ? `${basename(wb.path)} · Research Navigator`
        : "Research Navigator";
