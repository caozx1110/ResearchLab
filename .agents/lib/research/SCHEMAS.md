...
- **Commit-bound guard**：`journaled_op` / `mutation_transaction` 可接收不序列化、无副作用的 fail-closed validator；它在 context body 返回后、`commit_op` 前作为最后一道进程内校验执行。失败必须复用普通异常路径，恢复 exact before-image/mode、留下 abort journal、不得 checkpoint，并原样传播 validator 异常。guard 仍基于 cooperative workspace/exact-target locking；不能宣称可原子排斥不遵守同一锁协议的外部 filesystem writer。
