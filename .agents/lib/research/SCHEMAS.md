...
- **完整 root rewind 链**：公开 restore catalog 与 `kb undo` 只选择尚未消费、`undoable=true` 的 root business operation；选中后，内部 rewind interval 必须从该 root 起覆盖其后全部 changed committed root，包括 `undoable=false` bookkeeping、已被早先 recovery 消费的 business root 与 recovery root。child/descendant 不单独回放，因为 authoritative root before-image 已覆盖其 mutation。系统先锁住完整 target union，再逆序证明每段 `after_digests → before_digests`，成功后只给本轮仍可选 business root 写 `undone_by`；链不连续时以“未记账修改或日志损坏”中性分类零写拒绝。
