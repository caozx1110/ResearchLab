...
def skills_root(start: Path | None = None, *, explicit_home: str | Path | None = None) -> Path:
    ...
    if explicit:
        ...
        _product_skills_tree(candidate)
        return candidate
    ...
    if start is not None:
        ...
        for candidate in (search_from, *search_from.parents):
            try:
                ...
                continue
            return candidate
        raise FileNotFoundError(f"Could not locate product skills from {current}")
    runtime_file = Path(__file__).resolve()
    container = runtime_file.parents[2]
    if container.name == "runtime":
        candidate = container.parent
    elif container.name == ".agents":
        candidate = container.parent
    else:
        raise FileNotFoundError(f"Could not identify product layout from {runtime_file}")
    _product_skills_tree(candidate)
    return candidate


def _product_skills_tree(bundle_root: Path) -> Path:
    source_tree = bundle_root / "skills"
    source_runtime = bundle_root / "runtime" / "lib" / "research"
    if (source_tree / "metadata.yaml").is_file() and (source_runtime / "__init__.py").is_file():
        return source_tree
    installed_tree = bundle_root / ".agents" / "skills"
    installed_runtime = bundle_root / ".agents" / "lib" / "research"
    if (installed_tree / "metadata.yaml").is_file() and (installed_runtime / "__init__.py").is_file():
        return installed_tree
    raise FileNotFoundError(f"Could not locate product skills below {bundle_root}")


def resolve_skill_script_path(
    relative_path: str,
    *,
    explicit_home: str | Path | None = None,
    require_file: bool = True,
) -> Path:
    """Resolve an installed-form skill route against the loaded product bundle.

    Product routes intentionally retain the portable ``.agents/skills/...``
    spelling.  A source checkout maps that route into top-level ``skills/``;
    an installed workspace maps it into ``.agents/skills/``.  The target KB
    workspace is never searched, so a same-named local skill cannot shadow the
    product implementation.
    """

    route = Path(relative_path)
    if route.is_absolute() or route.parts[:2] != (".agents", "skills") or ".." in route.parts:
        raise ValueError("skill route must stay below .agents/skills")
    root = skills_root(explicit_home=explicit_home)
    tree = _product_skills_tree(root).resolve()
    candidate = (tree.joinpath(*route.parts[2:])).resolve(strict=False)
    try:
        candidate.relative_to(tree)
    except ValueError as exc:
        raise ValueError("skill route escaped the product skill tree") from exc
    if require_file and not candidate.is_file():
        raise FileNotFoundError(f"Product skill script not found: {relative_path}")
    return candidate
...
def skill_script_for_command(relative_path: str, *, cwd: Path | None = None) -> str:
    try:
        root = skills_root()
        script = resolve_skill_script_path(relative_path, require_file=False)
    except (FileNotFoundError, ValueError):
        return relative_path
    if str(os.getenv("RESEARCH_SKILLS_HOME") or "").strip():
        return script.as_posix()
    if (root / "runtime" / "lib" / "research").is_dir():
        return relative_path
