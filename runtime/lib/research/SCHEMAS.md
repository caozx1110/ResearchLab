...
Base presentation reset uses `research-kb-obsidian-base-presentation-reset/v1` as a private Agent preview binding; it is not a new public verb or a durable preference. Eligibility is restricted to renderer-owned dashboard `.base` files whose bounded strict-YAML payload exactly equals the unique Obsidian 1.12.7 GUI serialization constructed from current renderer bytes with only `views[i].sort` inserted at the fixture-defined mapping positions. Each sort item is rebuilt in exact `property/direction` order, has an `ASC|DESC` direction, and names a unique property already present in that view's `order`; comments, reordered mappings, and alternate serialization remain protected drift. The manifest snapshot must likewise exactly equal the unique renderer manifest rebuilt with its current `generated_at`, including the renderer-ordered `files` mapping. The zero-write preview first validates journal envelopes and refuses incomplete/quarantined state, then generates a random token whose digest binds renderer revision, canonical input digest, exact renderer-derived manifest bytes/file set/digests, current/desired Base digests, file and ancestor identities, exact target set, sort count, and the last committed journal anchor. Apply requires the token, matching digest and current-message authorization, then repeats journal and preview checks under the workspace lease. The derived transaction uses anchored `dir_fd` atomic exchange (`renameat2(RENAME_EXCHANGE)` on Linux or `renameatx_np(RENAME_SWAP)` on macOS) between the selected Base and a staging inode under the current operation's existing private `.journal/snapshots/<op_id>/` recovery directory; unsupported platforms fail closed without ordinary-rename fallback. The staging directory is bound to its opened directory identity and is freshly reopened through the anchored journal path before preparation, immediately before exchange, after exchange before unlink, and during conflict recovery; path detachment is a conflict, while the pinned descriptor remains the sole authority for recovering the displaced inode. Staging is removed after normal success or handled conflict, never enters the managed tree, and on a real crash remains ordinary journal recovery material so resume restores the Base without a managed unowned artifact. If exchange-time file, ancestor, or staging-directory validation fails, the exact displaced inode is retained through journal abort and restored atomically afterward without writing through a replacement ancestor or replacement staging path. A successful reset commit or later undo advances the committed-journal anchor and rejects replay; a rollback that changes file identity also requires a fresh preview. No separate consumed ledger or runtime authority file is created. Unknown/mixed/semantic drift, stale or spoofed manifests, replay, and unowned, symlink, special or unsafe paths remain fail-closed; canonical and human-owned paths are never targets.
