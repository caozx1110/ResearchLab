...
- **Durable abort CAS**：需要外部 leaf 并发保护的 root 在 `begin_op` 首次 durable write 中写 `abort_cas.schema=research-kb-abort-cas/v1`，且 `targets` key set 与 `target_paths` 精确相等。每个 target 从 `unarmed` 开始；同一次 begin write 已 no-follow 捕获 exact `before_leaf`、target parent capabilities 与固定 `snapshots/<op_id>` staging directory capability，并在 body 前复核它们，不能到 arm 才补录。journal API 后续自行捕获 owned/displaced full leaf state，consumer 不得传入 authority。full state 至少绑定 kind、device/inode、file-type+permission mode、link count、device identity、size、mtime 与适用的有界 content/link digest；跨 rename 比较不使用 ctime，FIFO/socket/device 不 open。staging leaf/data/directory 必须先 fsync，随后以 authoritative journal-byte digest CAS 持久化 `prepared`；arm 前及 exchange 前后都要求 exact before/owned identity，same-bytes/different-inode 是第三方 conflict。exchange 后双侧 exact compare 成功才写 `validated`；任一 mismatch 必须立即反向 exchange，反向不 exact 则保留 material 并 quarantine。validated staging 再次 exact 验证并 fsync 后写 `retained`，不 unlink displaced before leaf；它在私有 snapshot path 与 before snapshot 同生命周期保留。所有 guarded target `retained` 后才允许 commit。abort/resume 先全量分类再写：target=before 时零 churn；prepared+target owned+staging exact 时 exchange 恢复 displaced full state；prepared+staging missing/rebound、任意 third-party state或不可证 authority 均零业务写保持 `abort_failed`；validated/retained 且 target exact-owned时优先用 retained state，仅在 durable validated 已证明 exact before 而 staging 后来不可用时才可按 frozen matrix使用 before snapshot。`begin|abort_failed` 都阻塞普通 mutation、preview 与 checkpoint。guarded `abort_failed` 可在 authority 重新可证时 resume 并 terminalize 为 abort；旧 Base-reset begin/abort_failed 无 guard/`before_leaf` 时拒绝自动恢复，其他 legacy begin 保持兼容，legacy abort_failed 纳入 quarantine。历史 commit/abort 和 committed undo/restore 忽略 recovery-only guard，after-digest rewind chain 不变。
