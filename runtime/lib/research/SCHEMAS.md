...
核心 hard imports 固定为 `yaml / markdownify / bs4`。`.agents/requirements.txt` 是随安装分发的完整精确 runtime lock，根 `requirements.txt` 必须保持同一 package/version 集合。`kb help` 与 `kb doctor` 在没有任何 core-ready interpreter 时走 stdlib-only read-only rescue，不创建 `.venv`、不运行 pip、不联网；业务 verb 仍可授权 workspace-managed provision。installer smoke 若离线 provision 失败，必须保留已安装 managed files、明确分类 runtime incomplete 并让 help/doctor 可达，不能把已完成文件安装伪装成全量回滚或要求运行不可启动的 doctor。
...
- **自签名 heuristic**：`confirmed_by` 与 `identity.default_confirmed_by` 共用一个 NFKC + casefold predicate。通用 AI/tool token、已知中文 AI marker、以及只由 Claude/Sonnet/Opus/Haiku/Fable model token + release/version token 组成的复合身份均拒绝；出现额外可能真人姓名 token（如 `Claude Martin`）可通过这一层，但不等于身份认证。当前消息 `user_authorization` attestation、逐字 evidence、实质门与 current receipt 仍独立强制；`kb init` 必须在任何偏好写入前调用同一 predicate。
...
- **claim 状态投影**：确认后的 canonical claim bytes 继续保留 verify 时的原始 `confirmation_status=pending_user_confirmation`，不得为显示“已确认”而回写 claim，否则会使 verification/content digest 与刚签发的 receipt 立即失效。唯一确认权威是 top-level current `ConfirmationReceipt`；公开 review、report、index 等 consumer 只对 receipt 覆盖且 evidence current 的 claim 投影为已确认，同时保留其原 `claim_type`。
- **恢复 after-state CAS**：已 commit operation 的 undo/restore 在 workspace + exact-target locks 内，要求 `after_digests` 完整覆盖原 journal target set；canonical target 的当前 digest 必须与完整 rewind 链一致。ordinary exact-key 不匹配在创建 recovery journal 前零业务写 fail-closed；目录/descendant scope 重叠时，无法从单一 hash 推导的父/子摘要延迟到外层 recovery transaction 内逐 root 重验，失败则精确回滚到调用前 bytes。`state=begin` 的 crash resume 仍按 root before snapshot 自愈，不适用 commit after-state CAS。
- **完整 root rewind 链**：公开 restore catalog 与 `kb undo` 只选择尚未消费、`undoable=true` 的 root business operation；选中后，内部 rewind interval 必须从该 root 起覆盖其后全部 changed committed root，包括 `undoable=false` bookkeeping、已被早先 recovery 消费的 business root 与 recovery root。child/descendant 不单独回放，因为 authoritative root before-image 已覆盖其 mutation。系统先锁住完整 target union，预证所有 exact-key 段，再以折叠到 topmost non-overlapping envelope 的单一 recovery journal 原子包住逆序回放；每个 root 写前按真实树重验 after-state，任一步失败由外层 before-image 全量回滚且不 checkpoint/mark consumed。成功后只给本轮仍可选 business root 写 `undone_by`；链不连续以“未记账修改或日志损坏”中性分类拒绝。
- **Recovery checkpoint 不得再物化 workspace**：成功 rewind 的 checkpoint 只使用本轮 restored canonical paths 与既有 KB Git repo，禁止调用 `ensure_workspace()` 或 auto-init repo。撤销 `initialize_workspace` 后，`.git/`、`.journal/` 与 lock/snapshot runtime 必须保留恢复历史，`.gitignore` 可作为唯一工作树基础设施保留/补齐以继续忽略它们；操作前 absent 的 config/user/index/unit 文件不得被 checkpoint 尾处理重新生成，删除本身按 exact pathspec 提交且 Git clean。
- **派生 passage cache 边界**：所有实际调用 `build_index()` 的命令 root 都必须把 `.runtime/search/passages.sqlite3` 纳入自己的 transaction target envelope，使本命令 crash/abort 可回滚；它仍不进 Git/checkpoint。显式历史 undo/restore 校验 journal envelope 完整性后只以 canonical keys 做 after-state CAS/before-image 回放，兼容旧版本漏记的 cache refresh；cache 本身进入外层 recovery envelope，canonical 回放全部成功后同事务 durable remove，任一步失败则恢复调用前 cache。cache 不出现在公开 restored paths，也不得在资料撤销后残留派生文本。
...
- **Special-file nonblocking**：journal digest/snapshot/abort/restore 必须以 `lstat` 分类且有界处理 filesystem node。普通文件才可读取 bytes；symlink 只读 link target；目录递归时遇 FIFO/socket/device 等特殊 child 只能记录类型/identity sentinel，绝不 `open`。begin snapshot 的既存特殊 target fail-closed。使用 `abort_cas/v1` 的 transaction 若中途看到第三方 special/absent/symlink/regular state，必须零业务写并进入 `abort_failed` quarantine；只有 exact-owned transition 可 exchange/recover，任何 leaf 都不能在缺少 exact-inode conditional syscall 时经“先检查再 unlink”删除。历史无 guard transaction 保持原兼容读取语义，但不得被宣称具备 full-state CAS。
- **Durable abort CAS**：需要外部 leaf 并发保护的 root 在 `begin_op` 首次 durable write 中写 `abort_cas.schema=research-kb-abort-cas/v1`，且 `targets` key set 与 `target_paths` 精确相等；所有 op（包括 absent-only）都先 durable 创建 `snapshots/<op_id>`，保证后续 journal byte-CAS 不依赖 before-image kind。每个 target 从 `unarmed` 开始；同一次 begin write 已 no-follow 捕获 exact `before_leaf` 与 target parent capabilities，并把 staging authority 保持为空。arm 才捕获固定 `snapshots/<op_id>` directory capability、staging leaf 及 owned/displaced full leaf state，并随 `prepared` 一起持久化；consumer 不得传入 authority。full state 至少绑定 kind、device/inode、file-type+permission mode、link count、device identity、size、mtime 与适用的有界 content/link digest；跨 rename 比较不使用 ctime，FIFO/socket/device 不 open。staging leaf/data/directory 必须先 fsync，随后以 authoritative journal-byte digest CAS 持久化 `prepared`；arm 前及 exchange 前后都要求 exact before/owned identity，same-bytes/different-inode 是第三方 conflict。exchange 后双侧 exact compare 成功才写 `validated`；只有两侧仍为 durable expected pair、但 capability/currentness 失败时才可反向 exchange，任一 observed third-party 或 ambiguous leaf 不再交换并直接保留 material/quarantine。retain 对 exact displaced regular leaf 先 no-follow open、file-fsync、identity revalidate，再 fsync staging directory；symlink/FIFO/special 只 lstat、绝不 open。完成后才写 `retained`，且不 unlink displaced before leaf；它在私有 snapshot path 与 before snapshot 同生命周期保留。所有 guarded target `retained` 后才允许 commit。abort/resume 先全量分类再写，并在每个 recovery syscall 前重新验证 target/staging 仍为 durable exact pair：target=before 时零 churn；prepared+target owned+staging exact 时 exchange 恢复 displaced full state；prepared+staging missing/rebound、任意 third-party state或不可证 authority 均零业务写保持 `abort_failed`。`begin|abort_failed` 都阻塞普通 mutation、preview 与 checkpoint。guarded `abort_failed` 可在 authority 重新可证时 resume 并 terminalize 为 abort；旧 Base-reset begin/abort_failed 无 guard/`before_leaf` 时拒绝自动恢复，其他 legacy begin 保持兼容，legacy abort_failed 纳入 quarantine。历史 commit/abort 和 committed undo/restore 忽略 recovery-only guard，after-digest rewind chain 不变。
...
Base presentation reset uses `research-kb-obsidian-base-presentation-reset/v1` as a private Agent preview binding; it is not a new public verb or a durable preference. Eligibility is restricted to renderer-owned dashboard `.base` files whose bounded strict-YAML payload exactly equals the unique Obsidian 1.12.7 GUI serialization constructed from current renderer bytes with only `views[i].sort` inserted at the fixture-defined mapping positions. Each sort item is rebuilt in exact `property/direction` order, has an `ASC|DESC` direction, and names a unique property already present in that view's `order`; comments, reordered mappings, and alternate serialization remain protected drift. The manifest snapshot must likewise exactly equal the unique renderer manifest rebuilt with its current `generated_at`, including the renderer-ordered `files` mapping. The zero-write preview validates journal envelopes and refuses `begin`, `abort_failed`, or otherwise quarantined state, then generates a random token whose digest binds renderer revision, canonical input digest, exact renderer-derived manifest bytes/file set/digests, current/desired Base digests, file and ancestor identities, exact target set, sort count, and the last committed journal anchor. Apply requires the token, matching digest and current-message authorization, then repeats journal and preview checks under the workspace lease.

The derived transaction creates `abort_cas/v1` guards for the exact Base target set in the first durable root `begin` write, including each exact `before_leaf` and target-parent capability; every operation already has a durable private `snapshots/<op_id>` directory, but staging authority is empty until arm captures that directory capability, staging leaf, and owned/displaced full node state in durable `prepared`. The consumer stages and arms every target before the first business exchange, then exchanges the full set and finally retains the full set. Journal APIs, not the Obsidian consumer, capture authority, CAS each guard phase, execute the platform atomic exchange, and validate both sides after exchange. Only an exact durable expected pair may be reversed when capability/currentness validation fails; an observed third-party or ambiguous leaf remains in its compare-after position and quarantines the operation without another exchange. Unsupported platforms fail closed without ordinary-rename fallback. Crash/resume follows the durable `unarmed/prepared/validated/retained` matrix and revalidates both leaves immediately before every recovery syscall: exact-owned transitions can restore the displaced absent/regular/symlink/directory/special state, while same-bytes/different-inode or other third-party target state, missing/rebound staging, failed exact-pair reverse exchange, or incomplete authority remains `abort_failed` with zero speculative business write. Retain no-follow opens, file-fsyncs, and revalidates an exact regular displaced leaf before the staging-directory fsync; symlink/FIFO/special leaves remain lstat-only. Validated displaced before leaves are not unlinked; they remain in the private snapshot directory with the before snapshot as recovery material, and commit requires every target to be `retained`. No in-memory callback, generic before-image fallback, replacement staging path, bytes-only comparison, or check-then-unlink may claim recovery. A successful reset commit or later undo advances the committed-journal anchor and rejects replay; a rollback that changes file identity also requires a fresh preview. Historical committed undo ignores the guard, while legacy Base-reset begin/abort_failed without it is not automatically restored. No separate consumed ledger or runtime authority file is created. Unknown/mixed/semantic drift, stale or spoofed manifests, replay, and unowned, symlink, special or unsafe paths remain fail-closed; canonical and human-owned paths are never targets.
...
- `weekly` fill 为 `report-editorial-fill/v1`，固定有序 `executive_summary/progress/problems_and_risks/next_steps` 四区；owner scaffold 固定 lifecycle status，并按四区预置合法 `synthesis/fact/risk/plan` label，Agent 只填写正文与 refs。每条 `{text,refs,epistemic_label}` 都必须非空、引用 current catalog；risk ref 只能用于 `problems_and_risks` 且 label=`risk`。成品默认中文“本周摘要 / 进展 / 问题与风险 / 下周计划”，以 program title/question 为标题、自然语言展示 epistemic label、用稳定脚注序号代替 raw catalog ref，并机械附按“已确认决策 / 研究结论与证据 / 事实进展”分组的完整 evidence appendix；每个空组必须显式显示自然语言缺失项。
...
- `analyze`/`review` 的普通 prepare 继续拒绝覆盖 nonempty fill。只有显式 corpus refresh 可在旧 owner anchor、orientation 和 immutable fill projection 全部 current 时，保留白名单 Agent 字段（reviewer、selection rank、claim text/evidence refs），重建 corpus→orientation→anchor→preference context。v2 orientation 必须保存 owner 生成的 `static_scaffold_digest`；未知字段、immutable tamper、symlink、stale/missing anchor 均在 journal 前零写拒绝。refresh 只扩展可引用边界，不修改 claim 语义或 confirmation 状态。
