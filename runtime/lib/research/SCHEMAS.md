...
- **Special-file nonblocking**：journal digest/snapshot/abort/restore 必须以 `lstat` 分类且有界处理 filesystem node。普通文件才可读取 bytes；symlink 只读 link target；目录递归时遇 FIFO/socket/device 等特殊 child 只能记录类型/identity sentinel，绝不 `open`。begin snapshot 的既存特殊 target fail-closed。使用 `abort_cas/v1` 的 transaction 若中途看到第三方 special/absent/symlink/regular state，必须零业务写并进入 `abort_failed` quarantine；只有 exact-owned state 可清理或恢复。历史无 guard transaction 保持原兼容读取语义，但不得被宣称具备 full-state CAS。
- **Durable abort CAS**：需要外部 leaf 并发保护的 root 在 `begin_op` 首次 durable write 中写 `abort_cas.schema=research-kb-abort-cas/v1`，且 `targets` key set 与 `target_paths` 精确相等。每个 target 从 `unarmed` 开始；journal API 自行 no-follow 捕获 target parent capability、固定 `snapshots/<op_id>` staging directory/leaf、owned/displaced full leaf state，consumer 不得传入 authority。full state 至少绑定 kind、device/inode、file-type+permission mode、link count、device identity、size、mtime 与适用的有界 content/link digest；跨 rename 比较不使用 ctime，FIFO/socket/device 不 open。staging leaf/data/directory 必须先 fsync，随后以 authoritative journal-byte digest CAS 持久化 `prepared`；exchange 后双侧 exact compare 成功才写 `validated`，exact-owned cleanup 与 staging-dir fsync 后才写 `cleaned`。任一 exchange 后 mismatch 必须立即反向 exchange；反向不 exact 则保留 material 并 quarantine，绝不能 unlink 新状态。所有 guarded target `cleaned` 后才允许 commit。abort/resume 先全量分类再写：target=before 时零 churn；prepared+target owned+staging exact 时 exchange 恢复 displaced full state；prepared+staging missing/rebound、任意 third-party state或不可证 authority 均零业务写保持 `abort_failed`；validated/cleaned 且 exact-owned时才可按 frozen matrix 使用 before snapshot。`begin|abort_failed` 都阻塞普通 mutation、preview 与 checkpoint。guarded `abort_failed` 可在 authority 重新可证时 resume 并 terminalize 为 abort；旧 Base-reset begin/abort_failed 无 guard 时拒绝自动恢复，其他 legacy begin 保持兼容，legacy abort_failed 纳入 quarantine。历史 commit/abort 和 committed undo/restore 忽略 recovery-only guard，after-digest rewind chain 不变。
...
The derived transaction creates `abort_cas/v1` guards for the exact Base target set in the first durable root `begin` write. Desired staging leaves live only under the operation's fixed private `snapshots/<op_id>` directory. Journal APIs, not the Obsidian consumer, capture directory capability and owned/displaced full node authority, CAS each guard phase, execute the platform atomic exchange, validate both sides after exchange, reverse any mismatch, and clean only exact-owned staging. Unsupported platforms fail closed without ordinary-rename fallback. Crash/resume follows the durable `unarmed/prepared/validated/cleaned` matrix: exact-owned transitions can restore the displaced absent/regular/symlink/directory/special state, while third-party target state, missing/rebound staging, failed reverse exchange, or incomplete authority remains `abort_failed` with zero business write. No in-memory callback, generic before-image fallback, replacement staging path, or bytes-only comparison may claim recovery. A successful reset commit or later undo advances the committed-journal anchor and rejects replay; a rollback that changes file identity also requires a fresh preview. Historical committed undo ignores the guard, while legacy Base-reset begin/abort_failed without it is not automatically restored. No separate consumed ledger or runtime authority file is created. Unknown/mixed/semantic drift, stale or spoofed manifests, replay, and unowned, symlink, special or unsafe paths remain fail-closed; canonical and human-owned paths are never targets.
