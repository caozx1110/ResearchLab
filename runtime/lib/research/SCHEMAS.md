...
Base presentation reset uses `research-kb-obsidian-base-presentation-reset/v1` as a private Agent preview binding; it is not a new public verb or a durable preference. Eligibility is restricted to renderer-owned dashboard `.base` files whose bounded strict-YAML payload and byte-canonical GUI serialization differ from the current renderer bytes only by `views[i].sort`; comments and alternate serialization remain protected drift. Every sort item has exact keys `property/direction`, an `ASC|DESC` direction, and a unique property already present in that view's `order`; removing all such sort keys must make the complete payload equal. The zero-write preview generates a random one-time token; its digest binds that token, renderer revision, canonical input digest, exact renderer-derived manifest shape/file set/digests, manifest bytes, current/desired Base digests, file and ancestor identities, exact target set, sort count, the private consumed-ledger version and the last committed journal anchor. Apply requires the token, matching digest and current-message authorization, then re-previews under the workspace lease. In the same derived transaction it creates a `0600` consumed tombstone below the no-follow `0700` private runtime ledger and uses anchored parent `dir_fd` compare-and-replace for only the selected Base snapshots before ordinary update/status. The ledger is runtime/versioning-excluded but journaled with the reset; undo cannot revive an old authorization because the committed-journal anchor advances. Unknown/mixed/semantic drift, stale or spoofed manifests, replay, and unowned, symlink, special or unsafe paths remain fail-closed; canonical and human-owned paths are never targets.
