...
Base presentation reset uses `research-kb-obsidian-base-presentation-reset/v1` as a private Agent preview binding; it is not a new public verb or a durable preference. Eligibility is restricted to renderer-owned dashboard `.base` files whose bounded strict-YAML payload differs from the current renderer payload only by `views[i].sort`. Every sort item has exact keys `property/direction`, an `ASC|DESC` direction, and a unique property already present in that view's `order`; removing all such sort keys must make the complete payload equal. The digest binds renderer revision, canonical input digest, manifest bytes, current/desired Base digests, exact file set and sort count. Apply requires current-message authorization, re-previews under exact-path locks, writes only those Base files in one derived journal transaction, then runs ordinary update/status. Unknown/mixed/semantic drift and stale, unowned, symlink, special or unsafe paths remain fail-closed; canonical and human-owned paths are never targets.
