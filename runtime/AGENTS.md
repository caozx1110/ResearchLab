...
- Exact Base `views[i].sort` drift may be reset only from a zero-write, one-time token/digest preview explicitly authorized in the current message. Apply consumes it in the same journaled anchored-CAS transaction; replay, comments, manifest spoofing, mixed/semantic drift, and unsafe/unowned paths fail closed. Ordinary or post-intake refresh never resets it.
