...
- Exact Base `views[i].sort` drift may be reset only when raw Base and manifest bytes match the unique renderer/Obsidian 1.12.7 serialization, from a zero-write token/digest preview explicitly authorized in the current message. Preview rejects incomplete or quarantined journals; apply rechecks under the workspace lease and uses anchored Linux/macOS atomic exchange with no ordinary-rename fallback. Successful-commit/undo replay, comments, alternate serialization, manifest spoofing, mixed/semantic drift, and unsafe/unowned paths fail closed. Ordinary or post-intake refresh never resets it.
