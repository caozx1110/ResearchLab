...
- Exact Base `views[i].sort` drift may be reset only when raw Base and manifest bytes match the unique renderer/Obsidian 1.12.7 serialization, from a zero-write token/digest preview explicitly authorized in the current message. Preview rejects `begin`, `abort_failed`, or otherwise quarantined journals. Apply rechecks under the workspace lease and uses a durable `abort_cas/v1` guard plus anchored Linux/macOS atomic exchange with no ordinary-rename fallback. The first begin write owns exact `before_leaf` and parent/staging capabilities; journal APIs then persist `unarmed/prepared/validated/retained`, validate both sides after exchange, reverse mismatch, retain displaced before material without unsafe unlink, and quarantine same-bytes/new-inode, third-party, or unprovable states without generic overwrite. Successful-commit/undo replay, comments, alternate serialization, manifest spoofing, mixed/semantic drift, and unsafe/unowned paths fail closed. Ordinary or post-intake refresh never resets it.
