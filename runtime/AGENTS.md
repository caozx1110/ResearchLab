...
- Canonical records/programs/taxonomy/evidence are SSOT; managed Bases/views never edit them, human notes, or `.obsidian/`. Reading view only.
- Base reset only for raw Base+manifest matching unique renderer/1.12.7 serialization with exact `views[i].sort` drift. Apply needs current-message authorization bound to the zero-write preview; preview rejects `begin`/`abort_failed`/quarantine. `abort_cas/v1` begin binds before identity/parent caps; arm persists staging cap/leaf + full owned/displaced; all targets stage+arm before native-only exchange. Validate both sides after exchange; reverse only an exact pair on capability/currentness failure, else leave third-party/ambiguous state quarantined. Retain without unlink; commit requires all retained. Incomplete/legacy/replay plus semantic/unsafe/unowned drift and ordinary/post-intake refresh fail closed.
- Human-note intake: current-message-selected Markdown basename under `inbox/` or `annotations/`; never sweep/recency-pick. Reject review/nested/symlink/special/non-UTF-8/oversized input. Preserve bytes; freeze `source_origin=human-note` blog; normal fill/verify/confirmation.
- Obsidian checkboxes are drafts: later parse read-only, restate choices, get current-message authorization, apply batch atomically. Projection never consumes sheets.
