...
- Exact Base `views[i].sort` drift may be reset only when raw Base and manifest bytes match the unique renderer/Obsidian 1.12.7 serialization, from a zero-write token/digest preview explicitly authorized in the current message. Preview rejects `begin`, `abort_failed`, or otherwise quarantined journals. Apply rechecks under the workspace lease and uses a durable `abort_cas/v1` guard plus anchored Linux/macOS atomic exchange with no ordinary-rename fallback. The first begin write owns exact `before_leaf` and target-parent capabilities; arm captures staging capability/leaf and full owned/displaced state in durable `prepared`. Apply arms the full target set before the first business exchange. Journal APIs validate both sides after exchange and reverse only an exact durable pair whose capability/currentness check failed; third-party or ambiguous leaves remain in place with recovery material and `abort_failed` quarantine. Displaced before material is retained without unsafe unlink. Successful-commit/undo replay, comments, alternate serialization, manifest spoofing, mixed/semantic drift, and unsafe/unowned paths fail closed. Ordinary or post-intake refresh never resets it.
