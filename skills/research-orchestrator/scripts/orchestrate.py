...
def resolve_auto_script_tokens(parts: list[str]) -> list[str]:
    command = list(parts)
    for index, part in enumerate(command):
        normalized = part.replace(os.sep, "/")
        for logical_route in ROOT_AWARE_AUTO_SCRIPTS:
            if normalized == logical_route or normalized.endswith(f"/{logical_route}"):
                command[index] = str(resolve_skill_script_path(logical_route))
                break
    return command
...
def execute_auto_plan(root: Path, plan: dict[str, Any]) -> int:
    ...
    child_command = resolve_auto_script_tokens(command_with_project_root(command_parts, root))
