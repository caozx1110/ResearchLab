...
经官方桌面端的隔离 GUI fixture 验证，用户为 renderer-owned Base 设置展示排序时只会增加 `views[i].sort`。系统仅在 bounded strict YAML、exact renderer-derived manifest、byte-canonical GUI serialization、renderer/input currentness、no-follow 文件与祖先身份和 exact schema allowlist 同时成立，并且移除 `sort` 后当前 payload 与 renderer payload 完全相等时，把它单独分类为展示排序漂移；注释或其它序列化差异仍是受保护漂移。普通 update 与入库后的 best-effort refresh 仍拒绝覆盖。Agent 的脱敏预览保持零写并生成一次性随机 token，digest 同时绑定当前文件身份、私有 consumed-ledger 版本与最后 committed journal anchor；只有当前消息明确授权且 token/digest/CAS 仍 current，才在独立 derived journal transaction 中以 anchored `dir_fd` compare-and-replace 重置对应 Base，并原子写入 consumed tombstone，再执行普通刷新与状态校验。旧预览即使相同排序再次出现或执行过 undo 也不能重放。`order`、filter/query、groupBy、name/type、properties/formulas/summaries、未知键、混合漂移、manifest ownership 伪装、unowned/symlink/special/unsafe path 一律保持 fail closed；这个流程不保存排序偏好，也不修改 canonical record、人工区域或 `.obsidian/`。
