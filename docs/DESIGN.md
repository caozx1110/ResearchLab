...
经官方桌面端的隔离 GUI fixture 验证，用户为 renderer-owned Base 设置展示排序时只会按 1.12.7 的固定序列化位置增加 `views[i].sort`。系统从 renderer desired payload 重建唯一允许的 GUI bytes（包括 view key 与 sort-item key 顺序），并要求 current raw bytes 精确相等；manifest 也必须用 current `generated_at` 与 renderer inputs 重建唯一 bytes，不能由 current payload 自行 dump 证明 canonical。只有 bounded strict YAML、renderer/input currentness、no-follow 文件与祖先身份及 exact schema allowlist 同时成立，才把差异单独分类为展示排序漂移；注释、mapping 重排或其它序列化差异仍是受保护漂移。普通 update 与入库后的 best-effort refresh 仍拒绝覆盖。Agent 的脱敏预览保持零写并生成随机 token，digest 同时绑定 exact manifest/Base bytes、文件与祖先身份及最后 committed journal anchor；`begin`、`abort_failed`、quarantined 或 malformed journal 状态拒绝签发。只有当前消息明确授权且 unlocked/locked 两次 token/digest/CAS 都 current，才在独立 derived journal transaction 中用 anchored `dir_fd` 的 Linux/macOS 原子 exchange 重置对应 Base；不支持原子 exchange 的平台 fail closed，不回退为普通 rename。

Base reset 在 root `begin` 首次持久化时为 exact target set 建立 `abort_cas/v1` guard；同一次 durable write 已 no-follow 捕获每个 target 的 exact `before_leaf` 与 parent capabilities，防止 arm 把 same-bytes/different-inode 替换误认成 before。后续在任何业务边界前依次持久化 `unarmed → prepared → validated → retained`。具名 staging inode 只创建在当前 op 的私有 `.journal/snapshots/<op_id>/` 恢复目录；arm 时由 journal 捕获 staging capability 与 owned/displaced absent/regular/symlink/directory/special full leaf state，consumer 不能提交 authority。多 target apply 必须先为全量 target 完成 stage+arm，静态冲突在第一次 business exchange 前失败；之后才全量 exchange，最后全量 retain。每次 guard 变化都 CAS journal bytes 并按 staging leaf、staging directory、journal parent 的顺序 fsync；只有 durable `prepared` 才能 exchange，所有 target `retained` 后才能 commit。每次 exchange 后同时比对两侧 full state；只有 compare-after 两侧仍是 durable expected pair、但 capability/currentness 复核失败时才能反向 exchange。任一 observed third-party 或 ambiguous leaf 都不再交换，而是保留当下位置与 recovery material 并进入 `abort_failed`。跨 Linux/macOS 没有 exact-inode conditional unlink，因此 validated displaced before leaf 不删除，而是在原私有 staging path 与 before snapshot 同生命周期保留；任何阶段都绝不 unlink 未证明属于本 op 的 leaf。崩溃后的 resume 使用同一 durable phase/state matrix：只恢复 exact-owned transition；第三方 absent、regular、symlink 或 special state、staging 缺失/重绑及不完整 authority 都零业务写并保持 quarantine，不能让 generic before-image 覆盖。成功 commit 或之后的 undo 会推进 committed anchor，因此旧预览即使相同排序再次出现也不能重放；失败恢复后若 inode 改变也必须获取 fresh preview。历史 committed undo 忽略 guard；旧 Base-reset begin/abort_failed 若无 guard 则拒绝自动恢复。`order`、filter/query、groupBy、name/type、properties/formulas/summaries、未知键、混合漂移、manifest ownership 伪装、unowned/symlink/special/unsafe path 一律保持 fail closed；这个流程不保存排序偏好，不创建额外 consumed ledger，也不修改 canonical record、人工区域或 `.obsidian/`。
...
需要与不协作的外部 filesystem writer 共存的 mutation 可声明 durable `abort_cas/v1` guard；guard 是未提交 root 的恢复 authority，不是新的 business/checkpoint authority。它在 `begin` 首写中覆盖 exact target set，并同时冻结 `before_leaf` 与 parent capabilities；后续用 journal-byte CAS 推进 phase，并以 full no-follow node state 证明 owned transition。自动 abort、resume 与 post-exchange retention 只交换 exact-owned state；看到第三方状态就保留现场并把 root 留在 `abort_failed` quarantine。validated displaced material 不做条件不足的 unlink，而在私有 journal staging 中保留。普通 mutation、preview 和 checkpoint 都把 `begin|abort_failed` root 视为未完成。无 guard 的历史 record 继续按原兼容合同读取，但不能被静默解释为具备更强 CAS 保证；committed undo/restore 的 rewind chain 和 after-digest 语义不因 guard 改变。
