...
Obsidian 1.12.7 在用户为 renderer-owned Base 设置展示排序时，会只增加 `views[i].sort`。系统仅在 bounded strict YAML、manifest/renderer/input currentness、no-follow 文件身份与 exact schema allowlist 同时成立，并且移除 `sort` 后当前 payload 与 renderer payload 完全相等时，把它单独分类为展示排序漂移。普通 update 与入库后的 best-effort refresh 仍拒绝覆盖；Agent 先给出脱敏预览，只有当前消息明确授权且 preview digest/CAS 仍 current，才在独立 derived journal transaction 中把对应 Base 原子重置为 renderer bytes，再执行普通刷新与状态校验。`order`、filter/query、groupBy、name/type、properties/formulas/summaries、未知键、混合漂移、unowned/symlink/special/unsafe path 一律保持 fail closed；这个流程不保存排序偏好，也不修改 canonical record、人工区域或 `.obsidian/`。
