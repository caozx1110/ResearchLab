...
需要与不协作的外部 filesystem writer 共存的 mutation 可声明 durable `abort_cas/v1` guard；guard 是未提交 root 的恢复 authority，不是新的 business/checkpoint authority。它在 `begin` 首写中覆盖 exact target set，用 journal-byte CAS 推进 phase，并以 full no-follow node state 证明 owned transition。自动 abort、resume 与 post-exchange cleanup 只改 exact-owned state；看到第三方状态就保留现场并把 root 留在 `abort_failed` quarantine。普通 mutation、preview 和 checkpoint 都把 `begin|abort_failed` root 视为未完成。无 guard 的历史 record 继续按原兼容合同读取，但不能被静默解释为具备更强 CAS 保证；committed undo/restore 的 rewind chain 和 after-digest 语义不因 guard 改变。
