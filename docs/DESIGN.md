...
新生成的 paper `note.md` 是 reader-first 派生阅读面，不改变 canonical claim/evidence/receipt：runtime Agent 依据证据选择 `method_system|benchmark|survey`，并完成七个共同维度与所选类型的全部增量维度。每个维度要么包含 summary 和一条或多条独立 judgement claim，要么以 evidence-bound `not_applicable` 明确说明为何不适用；脚本只验证精确矩阵、claim identity/type 和逐字证据，不用字数、关键词或 claim 数量评分。正文按共同维度、类型维度的固定顺序先渲染 summary 与全部独立 claim，每节只放一个稳定证据入口；文末保留每条 ref 的 source unit、artifact、原始 locator、完整 quote 与可选 summary，并用 Obsidian 原生默认折叠 callout 隔离逐字引文。`paper-deep-read/v2` 的正文投影与 claims 一起进入 verification/confirmation content digest；历史 schema-less v1 只读兼容且不被 prepare 覆盖。普通 Markdown 阅读器仍可顺序读出 callout 内容。动态文本全部按字面转义；历史 note 不因 projection refresh 或无关 analyzer 操作批量重写。
