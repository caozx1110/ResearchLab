...
经官方桌面端的隔离 GUI fixture 验证，用户为 renderer-owned Base 设置展示排序时只会按 1.12.7 的固定序列化位置增加 `views[i].sort`。系统从 renderer desired payload 重建唯一允许的 GUI bytes（包括 view key 与 sort-item key 顺序），并要求 current raw bytes 精确相等；manifest 也必须用 current `generated_at` 与 renderer inputs 重建唯一 bytes，不能由 current payload 自行 dump 证明 canonical。只有 bounded strict YAML、renderer/input currentness、no-follow 文件与祖先身份及 exact schema allowlist 同时成立，才把差异单独分类为展示排序漂移；注释、mapping 重排或其它序列化差异仍是受保护漂移。普通 update 与入库后的 best-effort refresh 仍拒绝覆盖。Agent 的脱敏预览保持零写并生成随机 token，digest 同时绑定 exact manifest/Base bytes、文件与祖先身份及最后 committed journal anchor；incomplete、quarantined 或 malformed journal 状态拒绝签发。只有当前消息明确授权且 unlocked/locked 两次 token/digest/CAS 都 current，才在独立 derived journal transaction 中用 anchored `dir_fd` 的 Linux/macOS 原子 exchange 重置对应 Base；不支持原子 exchange 的平台 fail closed，不回退为普通 rename。具名 exchange staging inode 只创建在当前 op 已有的私有 `.journal/snapshots/<op_id>/` 恢复目录，不进入 managed tree 或形成新的 runtime authority；该目录以打开时的目录身份绑定，并在准备前、exchange 前后及冲突恢复时从 anchored journal 路径重新打开比对，路径脱离即按冲突处理。正常成功/冲突显式删除 staging，进程崩溃时则作为既有 journal recovery material 保留，resume 可恢复 Base 且不会遗留 managed unowned file；即使可见 staging 路径已被替换，冲突恢复仍使用固定目录句柄取回 displaced inode。交换边界的并发 inode、祖先替换与 staging 目录脱离会在 journal abort 后按保留的 displaced inode 恢复，不能静默覆盖或写穿。成功 commit 或之后的 undo 会推进 committed anchor，因此旧预览即使相同排序再次出现也不能重放；失败恢复后若 inode 改变也必须获取 fresh preview。`order`、filter/query、groupBy、name/type、properties/formulas/summaries、未知键、混合漂移、manifest ownership 伪装、unowned/symlink/special/unsafe path 一律保持 fail closed；这个流程不保存排序偏好，不创建额外 consumed ledger，也不修改 canonical record、人工区域或 `.obsidian/`。
