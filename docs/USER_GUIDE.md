...
选择现在设置后，Agent 会在一个紧凑回合询问真实署名、语言与术语风格、研究方向、资源与重要约束，并显示版本记录节奏和论文自动初筛的当前值；你可以对默认项回复“默认即可”。资源会进入后续方法设计等流程可直接读取的资源画像，重要约束会追加去重并保留旧值。报告风格、协作边界和开发者诊断等低频项以后按需补充。真实署名只在第一次确认研究判断前强制，因此跳过不会让日常研究流程停住。
