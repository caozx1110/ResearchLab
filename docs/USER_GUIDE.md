...
如果你只在 Base 面板里设置了展示排序，Agent 可以先展示一个不改文件的安全修复预览。只有你在当前对话明确确认，且 Base、manifest、文件身份与恢复历史在锁内复核仍完全匹配后，它才会原子重置为生成视图并重新刷新；成功修复、撤销、文件身份变化或其它已提交操作都会使旧预览失效，需要重新预览。修复前会先持久记录精确恢复阶段和文件身份；崩溃后只恢复能证明属于本次操作的状态，遇到并发删除、链接、特殊文件或其它新状态会保留现场并暂停后续写入，等待安全恢复，不会用旧视图覆盖。未完成或异常的恢复日志也会先阻止预览。普通 `kb obsidian update` 和资料入库后的自动刷新不会自行覆盖这类变化。筛选、分组、列顺序、字段、注释、YAML 键顺序或其它人工修改仍会被当作受保护漂移保留，需人工判断。
