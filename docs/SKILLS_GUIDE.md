...
- 主题相关的短词、tag 规则、taxonomy seed、repo role 词表统一放在 `kb/memory/domain-profile.yaml`
...
这只是一个示例方向；如果你的领域不同，先更新 `kb/memory/domain-profile.yaml`，后面的 intake、tagging、trend survey 和 design ranking 会自动跟着新的主题配置走。
...
- `kb/library/landscapes/<survey-id>/landscape-report.yaml`
- `kb/library/landscapes/<survey-id>/summary.md`
...
- `kb/programs/<program-id>/charter.yaml`
- `kb/programs/<program-id>/workflow/state.yaml`
- `kb/programs/<program-id>/workflow/preferences.yaml`
- `kb/programs/<program-id>/workflow/open-questions.yaml`
...
```text
把这个 search result 里的 shortlisted URL 批量入库，并在过程中打印每条来源的进度和失败项：kb/library/search/results/<search-id>.yaml
```

生成内容：

- `kb/library/literature/<source-id>/metadata.yaml`
- `kb/library/literature/<source-id>/note.md`
- `kb/library/literature/<source-id>/claims.yaml`
- `kb/library/literature/<source-id>/methods.yaml`
- `kb/library/literature/index.yaml`
- `kb/library/literature/graph.yaml`
...
- `kb/library/literature/tags.yaml`
- `kb/library/literature/tag-taxonomy.yaml`
- 必要时更新 `kb/library/literature/<source-id>/metadata.yaml`
...
- `kb/library/repos/<repo-id>/repo-notes.md`
- `kb/library/repos/<repo-id>/entrypoints.yaml`
- `kb/library/repos/<repo-id>/modules.yaml`
- `kb/library/repos/index.yaml`
...
- `kb/programs/<program-id>/evidence/literature-map.yaml`
- 更新 `kb/programs/<program-id>/workflow/state.yaml`
...
- `kb/programs/<program-id>/ideas/index.yaml`
- `kb/programs/<program-id>/ideas/<idea-id>/proposal.yaml`
- 更新 `kb/programs/<program-id>/workflow/state.yaml`
...
- `kb/programs/<program-id>/ideas/<idea-id>/review.yaml`
- `kb/programs/<program-id>/ideas/<idea-id>/decision.yaml`
- `kb/programs/<program-id>/ideas/<idea-id>/review-assist.md`
- `kb/programs/<program-id>/ideas/<idea-id>/revision-assist.md`
- 更新 `kb/programs/<program-id>/ideas/index.yaml`
- 必要时更新 `kb/programs/<program-id>/workflow/evidence-requests.yaml`
- 更新 `kb/programs/<program-id>/workflow/state.yaml`
...
- `kb/programs/<program-id>/design/selected-idea.yaml`
- `kb/programs/<program-id>/design/repo-choice.yaml`
- `kb/programs/<program-id>/design/interfaces.yaml`
- `kb/programs/<program-id>/design/coordination-contracts.yaml`
- `kb/programs/<program-id>/design/system-design.md`
- `kb/programs/<program-id>/experiments/matrix.yaml`
- `kb/programs/<program-id>/experiments/runbook.md`
- 更新 `kb/programs/<program-id>/workflow/state.yaml`
...
```text
把当前 kb/ 里的共享 literature、repos、tags、landscapes 和 program 产物生成人类可读的本地网页，并在后台静默监听更新。
```

生成内容：

- `kb/user/kb/index.html`
- `kb/user/kb/snapshot.json`
- `kb/user/kb/assets/*`
- `kb/user/kb/.runtime/launcher-state.json`
- `kb/user/kb/.runtime/build-status.json`
...
作用：把 `kb/` 里的共享 literature、repos、tags、landscapes 和 program 级产物生成成本地可视化知识库 UI，并通过后台 daemon 静默监听更新、自动重建、自动刷新已打开页面。
