...
4. `retain_abort_cas` revalidates and fsyncs the exact displaced leaf in the private snapshot directory, then advances it to `retained`. It deliberately does not unlink. A guarded Base reset may commit only when all targets are retained and a final two-pass validation still sees every exact owned target, retained leaf, and directory capability.
