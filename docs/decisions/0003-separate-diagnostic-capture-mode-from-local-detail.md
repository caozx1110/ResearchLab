...
Automatic callers may submit only `diagnostic-mechanical-envelope/v1`: an allowlisted exception class and failure stage, at most eight repository-relative frames that resolve to the named function/line in an actual managed Python product file without persisting source text, at most four allowlisted event codes, the current Python version (or `unknown`), and a small allowlist of exact installed dependency versions. Source-checkout frames must byte-match the corresponding Git `HEAD` blob; installed frames must byte-match the exact path digest in the copy-project install manifest. Every source-path component is read through anchored no-follow descriptors, so untracked, locally modified, self-use, linked, special, or concurrently displaced files fail private detail closed. The dispatcher constructs this envelope independently of child arguments and captured stdout/stderr. The intake owner's one-use failure-stage receipt may refine an `unknown` dispatcher stage; a conflicting stage fails private detail closed and retains the legacy redacted record.
...
Summary and detail writes are one declared mutation transaction and roll back to their before-images together. Private directories and files use anchored no-follow traversal, regular-file/owner/mode/size checks, `0700` directories, and `0600` artifacts. A create requires the target to be absent; an update is compare-and-swap bound to the exact previously read bytes, so an orphan or concurrently changed artifact is never overwritten. A detected replacement conflict retains at most one bounded private recovery backup; a later write removes it only after exact visible/backup equality, otherwise manual inspection remains required. Private detail is hard-excluded from checkpoint path discovery even if a legacy file was force-added. Export preview, public output, Agent public projections, installer/update, sync, and versioning never read it.
