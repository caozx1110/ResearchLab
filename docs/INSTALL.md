...
`kb init` 会先创建可立即使用的知识库布局，再由 AI 询问要“现在设置”还是“先跳过”。快速设置只集中询问真实署名、语言与术语风格、研究方向、资源与重要约束；当前版本记录和论文初筛设置可以直接接受默认值。跳过不产生额外偏好写入，后续说“补充我的研究偏好”即可继续；真实署名只会在第一次确认研究判断前再次要求。是否创建终端快捷命令不影响这条对话式主路径。
