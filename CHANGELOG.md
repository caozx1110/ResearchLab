...
The accepted v2 product workflow is natural-language and routed across the five owners below. Issue #61 tree cleanup is still a review candidate, not merged default-branch fact until human review and merge. Older command, owner, and tool names retained later in this Unreleased section are historical release-candidate notes only and are superseded; they are not current instructions.
...
- The Issue #61 cleanup candidate removes retired v1 schema, migration, CLI, record, projection, PDF, and HTML helpers from the tracked source tree while preserving the installed four-file v2 runtime allowlist (`v2_bootstrap`, `legacy_detector`, `updater`, and the package marker) and PyYAML-only runtime lock. This source-tree outcome is not default-branch fact until human merge; the retired helpers are not installed in either state. Defuddle, AnyDoc, PDF, and other converters manage their own optional dependencies outside the bundle.
...
- Development collaboration is now GitHub-remote-complete: tracked design/ADR/owner contracts, Epic and Atomic Issues, pushed checkpoints, consolidated PRs, Actions evidence, and human review form the complete handoff chain. Ordinary changes use a lightweight path; frozen ownership, takeover approval, exact dependency binding, and release controls are added only when the risk requires them. Local workspaces, one-off prompts, tool memory, chats, stashes, and unpushed state are no longer workflow inputs or evidence.
- Historical pre-cutover source cleanup replaced retired private design section references with tracked `docs/DESIGN.md` and then-current v1 `SCHEMAS.md` anchors. That `SCHEMAS.md` role is superseded and is not a current v2 design or contract authority.
...
- Paper deep-read notes present analysis before any full quote, with complete evidence retained in deterministic, default-collapsed callouts. Page/section links share one fail-closed source-map resolver, honestly fall back to the Markdown document, and paper Obsidian pages expose an existing canonical deep-note entry without rewriting historical notes.
- New paper deep reads now use a versioned multidimensional matrix: seven common sections plus the selected method/system, benchmark, or survey sections. Every assessed section preserves one or more independent evidence-backed claims; genuinely inapplicable sections require a reviewable evidence-backed N/A reason. Expanded analysis is byte-bound into verification and confirmation currentness, while schema-less v1 notes retain read compatibility and their historical digest behavior.
