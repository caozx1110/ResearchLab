...
- Obsidian 1.12.7 Base `views[i].sort` changes now receive a distinct presentation-drift classification only when removing the exact allowlisted sort makes the current strict-YAML payload identical to renderer output. Ordinary refresh remains fail-closed; an Agent must present a preview and obtain current-message, digest-bound authorization before an atomic derived reset and revalidation. Mixed/semantic drift, unsafe paths, human areas, and canonical records remain untouched.
