...
- Obsidian 1.12.7 Base `views[i].sort` changes now receive a distinct presentation-drift classification only when the raw file exactly matches the unique renderer-derived GUI bytes with the allowlisted sort inserted; manifests likewise must equal the unique renderer rebuild, including mapping order. Ordinary refresh remains fail-closed. A zero-write Agent preview uses a random token bound to exact manifest/file identities and the last committed-journal anchor, and refuses incomplete or quarantined journal state. Current-message apply rechecks under the workspace lease and uses Linux/macOS anchored atomic exchange with no ordinary-rename fallback in one derived transaction. Its durable abort-CAS guard records exact begin-time `before_leaf`/target-parent capabilities, captures staging/full owned-displaced authority at arm, and arms the full target set before the first business exchange. Every operation now has a durable private CAS directory even for absent-only snapshots; recovery rechecks both leaves before each syscall, and retain file-fsyncs/revalidates exact regular displaced material while never opening symlink/FIFO/special leaves. Crash/resume restores only exact-owned transitions; compare-after reverses only an exact durable pair whose capability/currentness check failed, while third-party or ambiguous leaves remain in place with recovery material and `abort_failed` quarantine. Displaced before material is retained without unsafe unlink, and same-bytes/new-inode, concurrent absent/regular/symlink/special, or otherwise unprovable states never receive a generic overwrite. CLI preview/apply safety failures use fixed redacted output and protocol codes. A successful commit or undo advances the anchor and rejects replay; comments, alternate serialization, manifest ownership spoofing, mixed/semantic drift, unsafe paths, human areas, and canonical records remain untouched.
