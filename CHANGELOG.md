...
The current product workflow is natural-language, routed across the five owners below. Older command, owner, and tool names retained later in this Unreleased section are historical release-candidate notes only and are superseded; they are not current instructions.
...
- Installed runtime delivery now uses an explicit four-file v2 allowlist (`v2_bootstrap`, `legacy_detector`, `updater`, and the package marker) and a PyYAML-only runtime lock. Retired v1 schema, migration, CLI, record, projection, PDF, and HTML helpers are absent from the active source tree and are not installed; historical details are retained only in Git history and superseded notes. Defuddle, AnyDoc, PDF, and other converters manage their own optional dependencies outside the bundle.
