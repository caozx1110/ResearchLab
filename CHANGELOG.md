...
- Obsidian 1.12.7 Base `views[i].sort` changes now receive a distinct presentation-drift classification only when both the strict-YAML payload and byte-canonical GUI serialization match renderer output after removing the exact allowlisted sort. Ordinary refresh remains fail-closed; a zero-write Agent preview uses a one-time random token bound to exact manifest/file identities plus consumed-ledger and committed-journal anchors. Current-message apply consumes that token and resets through anchored `dir_fd` CAS in one recoverable derived transaction. Replay, comments, manifest ownership spoofing, mixed/semantic drift, unsafe paths, human areas, and canonical records remain untouched.
